...
export function Header() {
    ...
    return (
        <header className="relative mt-3 z-50 top-0 left-0 text-white font-inter mb-20 ">
            <div>
                <Link href={"/"} className="absolute top-0 left-0 h-20 -z-10 blur-md w-full">
                    <Image
                ...
                <div className="flex items-center 2xl:justify-evenly justify-between px-4 md:px-10">
                    ...
                    <div className="hidden xl:flex items-center space-x-4">
                        <div className="flex items-center space-x-3">
                            <Link href="/home/carrito" className="relative hover:text-purple-400"><Icon_Shopping_Cart />
                                {cart.map((item) => {
                                    return <p key={item.id} className='text-xs absolute -top-1 -right-3 px-2 py-1 rounded-full bg-white text-[#1F2937]'>{item.quantity}</p>
                                })}
                            </Link>
                            <Link href="/home/carrito" className="hover:text-purple-400"><Icon_Notification /></Link>
                            <Link href="/home/carrito" className="hover:text-purple-400"><Icon_Email /></Link>
                        </div>
                        <div className="flex items-center space-x-3">
                    ...
                    <nav className="xl:hidden w-[70%] md:w-[50%] bg-[#1F2026] text-sm space-y-4 py-4 px-6 fixed top-0 right-0 duration-300 h-screen overflow-auto md:z-10 max-md:z-10">
                        ...
                        <div className='bg-gray-300/15 rounded-md p-2 px-3'>
                            ...
                            <div className="flex items-center space-x-6 justify-center">
                                <Link href="/home/carrito" className="hover:text-purple-400 scale-150"><Icon_Shopping_Cart /></Link>
                                <Link href="/home/carrito" className="hover:text-purple-400 scale-150"><Icon_Notification /></Link>
                                <Link href="/home/carrito" className="hover:text-purple-400 scale-150"><Icon_Email /></Link>
                            </div>
                        </div>
                    </nav>
                )}
